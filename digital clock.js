...
let sequenceIndex = 0; // Index to keep track of current sequence
...
function toggleClockStopwatch() {
    ...
    switch (sequenceIndex) {
        case 0: // Displaying clock
            clockContainer.style.display = 'none';
            stopwatchContainer.style.display = 'block';
            sequenceIndex = 1;
            break;
        case 1: // Displaying stopwatch
            stopwatchContainer.style.display = 'none';
            timerContainer.style.display = 'block';
            sequenceIndex = 2;
            break;
        case 2: // Displaying timer
...
function setAlarm() {
    ...
    const alarmTime = new Date(currentTime.getFullYear(), currentTime.getMonth(), currentTime.getDate(), alarmHours, alarmMinutes, 0);

    const timeUntilAlarm = alarmTime - currentTime;
    if (timeUntilAlarm > 0) {
        alarmTimeout = setTimeout(() => {
            alert("Alarm! Your alarm has ended.");
        }, timeUntilAlarm);
    } else {
